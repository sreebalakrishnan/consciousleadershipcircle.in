...
/conversations/an-invitation  → conversations/an-invitation.html    Shareable **invitation page** — what it's like to sit for a conversation, written to *you* (the potential guest). Explains the spirit (no agenda, no prep, not a boardroom; we talk, and discard if we don't like it) and the practical shape (find a time → Google Meet scheduled ahead → ~1hr, can run a few mins over → few days of editing: transcribe + wire up → published in Conversations). Reuses the Sparks chat-bubble pattern (`.thread`/`.msg`/`.bubble`, with `.msg.me` = Sree's right-aligned green bubbles) to **rebuild Sree's real WhatsApp exchange with Siva** the day before recording — the "Heads-up on the theme?" → "no agenda… discard and start again" → "let the Sree magic unfold" / "May the inner Siva unfold" thread (joining links + phone numbers deliberately omitted). Has a numbered `.shape` list, two `.eg` example cards (Roopa, Tuhin), and an `.invite` CTA (WhatsApp Sree + mailto). Self-canonical, `summary` twitter card (no og:image). Linked from the conversations index `more-note`. Not listed as a `.conv-item` (it's a meta/invite page, not an episode).
...
/conversations/sivarama-sundar → conversations/sivarama-sundar.html    Conversation: Sree with Sivarama Sundar — "Three decades, five machines" (**audio-only**, 1 hr 41 min, June 2026). First audio episode; a new page pattern: Paris Review-style **edited interview** (Sree's questions in `.q` serif-italic blocks, Siva's answers as body prose) structured into five era sections ("five machines": dual-floppy 8086 1992–95 · Clipper years 1995–99 · Delphi decade 1999–2009 · gray shades 2009–23 · the machine that talks back 2023–); era-grouped chapter list (`.chapter-era` headings) whose rows **seek the embedded player inline** via YouTube iframe postMessage (`enablejsapi=1`), with watch?v= links as fallback; two verbatim `.duet` exchanges (Prince of Persia/Super Mario; the ₹750 paycheck); three `.gloss` "For the younger reader" asides (Borland/Norton, NIIT, floppy/TSR); a dark animated **DOS screen card** (`.dos`) recreating the raindrops virus (CSS-only falling characters, respects prefers-reduced-motion); an **afterword** section ("A few lines he wished he'd added") holding reflections Siva sent *after* listening to the draft — a `.coda` card ("In his words · added after") on learning from great early bosses by observation (incl. "what not to do, and how not to do it") and what he enjoyed/learnt working with founders; this keeps the recording-based prose honest by not injecting post-hoc lines into it (the only inline tweak from his feedback is the "I wanted to be a programmer — and I still am, an engineer at heart" echo in Era II). Then a `.coda` accepting Siva's proposal that the next episode adds Sesame's Maya as a third (non-human) voice; closing pull "Happy to leave any tiny breadcrumbs…"; guest card links his LinkedIn + his musing. YouTube video ID `NiKguM1S540` (https://www.youtube.com/watch?v=NiKguM1S540 — titled "From COBOL to AI: A 30-Year Journey Through Software"; set as the `VIDEO_ID` constant in the inline script, which drives both the embed and the inline chapter seeking). **Hero comic** at `/conversations/images/sivarama-sundar.jpg` (1536×1024, ~0.9MB JPEG q90; lightboxed `figure#comic` under the "five machines" framing para, with `onerror` hide-if-missing) + og:image/twitter:image (`summary_large_image`). Self-canonical.
